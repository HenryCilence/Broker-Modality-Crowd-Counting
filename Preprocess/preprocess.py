...
if __name__ == '__main__':
    ...
    for img_name in os.listdir(test_folder):
        ...
        if img_name.split(".")[1] != 'npy':
            continue
        img_name = img_name.split("_")[0] + "_"
        inf_img = image_read(os.path.join(test_folder, img_name + "T.jpg"), mode='GRAY')[
                      np.newaxis, np.newaxis, ...] / 255.0
        vis_img = image_read(os.path.join(test_folder, img_name + "RGB.jpg"), mode='GRAY')[
                      np.newaxis, np.newaxis, ...] / 255.0
        
        # For ShanghaiTechRGBD, normalization of depth is needed.

        # inf_img = np.load(os.path.join(test_folder, img_name + "T.npy"))
        # inf_img = cv2.resize(inf_img, size, interpolation=cv2.INTER_AREA)
        # inf_img = inf_img[np.newaxis, np.newaxis, ...]
        # inf_img /= inf_img.max()
        ...
        with torch.no_grad():
            sample = sample_fn(x_start=x_start, record=True, I=inf_img, V=vis_img, save_root=out_path,
                               img_index=os.path.splitext(img_name)[0], lamb=0.5, rho=0.001)
        ...
        imsave(os.path.join(os.path.join(out_path, 'recon'), "{}.png".format(img_name + "RGBT")),
